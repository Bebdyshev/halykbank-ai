...
Тайминги холодного прогона — см. секцию «Замеренные тайминги» (заполняется
после репетиции 8-го).
...
**ВНИМАНИЕ — захардкоженные пути:** `src/ledger.py` и `src/assemble.py` ждут
файл `master_ledger_2025.csv`. Если у нового леджера другое имя — либо
переименовать файл под ожидаемое, либо поправить одну строку в каждом.
(TODO после репетиции: заменить на glob `master_ledger*.csv`.)
